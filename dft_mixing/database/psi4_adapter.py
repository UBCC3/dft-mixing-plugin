...
class Psi4DbAdapter:
    ...
    def initialize_database(self, init_config_path):    
        ...
        db_path = init_config["db_path"]
        if os.path.exists(db_path):
            if os.path.isdir(db_path):
                raise RuntimeError("ERROR: DB path is pointing to a directory")
            root, ext = os.path.splitext(db_path)
            if ext != ".db":
                raise RuntimeError(f"Invalid extension {ext}")
            
        # Initialize database
        self.db = FunctionalDatabase(db_path, 
                                    init_config["external_resol"])

        # Already writes a file to database by default
        try: 
            ...
            for preload_dict in preload_items: 
                source = preload_dict["source"]
                type = preload_dict["type"]
                json_path = preload_dict["json_path"]
                
                with open(json_path, "r") as f:
                    preload_json = json.load(f)
                    
                if type == "MF":
                    self.load_multi_functional_data(preload_json, source)
                elif type == "BD":
                    self.load_base_dispersion_data(preload_json, source)
                elif type == "DC":
                    self.load_dispersion_config_data(preload_json, source)

        except Exception as e:
            os.remove(init_config["db_path"])
            raise e
    ...
    def bind_database(self, query_config_path: str):
        ...
        if (not os.path.exists(query_config["db_path"])):
            raise RuntimeError("ERROR: NO DATABASE FOUND!")
        
        self.db = FunctionalDatabase(
            query_config["db_path"],
            query_config["external_resol"]
        )
    
    def _import_dftd3_disps(self, dftd3_toml: str):    
        logger.warning("IMPORTING DFTD3 DISPERSIONS")
        
        toml_path = dftd3_toml
