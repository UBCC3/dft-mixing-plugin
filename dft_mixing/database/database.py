...
class FunctionalDatabase:
    ...
    def __init__(self, 
                 ...
                 external_resol: list = []):
        '''
            Creates a functional database wrapper from 
            a database path URI. If db_path points to a
            empty file, it creates a new one. 

            Can also specify a external resolution system, 
            with ordered priorities in `external_resol`.
            The names with lower index have higher priority.
        ''' 
        ...
        if (len(external_resol) > 0):
            while len(external_resol) != 0:
                last_priority = external_resol.pop(-1)
